"""EchoMimic-v2 web app (Refactored).
...
import sys
import torch
# We need to import this early to avoid local vs global scope issues in main() 
# if we try to import it conditionally inside the function.
try:
...
def main():
    ...
    # Load Pipeline
    pipe = load_pipeline(
        args.config, DEVICE, WEIGHT_DTYPE, _ECHOMIMIC_DIR,
    ...
    if args.compile_unet:
        ...
        print(f"[INIT] Compiling UNet starting (mode={args.compile_unet_mode})...")
        
        backend = "inductor" # Default for Linux
        
        # Windows-specific fix
        if os.name == 'nt':
            print("[WARN] Windows detected. Triton compiler might fail. Using default backend selection.")
            backend = None # Let PyTorch choose (often 'cudagraphs' or similar if inductor fails)
    ...
    posedata_args = (args.pose_dir, pose_files, args.width, args.height, DEVICE, WEIGHT_DTYPE)
    if args.low_ram:
    ...
    # Log directly to core/performance_logs
    ...
    try:
        with torch.no_grad():
            asyncio.run(run_server(pipe, ref_image, pose_provider, args, INDEX_HTML_PATH))
    finally:
        perf_monitor.stop()
